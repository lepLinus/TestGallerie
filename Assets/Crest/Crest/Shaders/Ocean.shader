	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 Frag(const Varyings input, const float facing : VFACE) : SV_Target
			{
				...
				float sceneZ01 = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uvDepth);
				...
				real3 col = OceanEmission(view, n_pixel, lightDir, input.foam_screenPosXYW.yzw, pixelZ, uvDepth, sceneZ, sceneZ01, bubbleCol, _Normals, _CameraDepthTexture, underwater, scatterCol);
