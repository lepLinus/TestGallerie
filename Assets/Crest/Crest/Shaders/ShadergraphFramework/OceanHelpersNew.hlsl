...
float2 WorldToUV(in float2 i_samplePos, in float3 i_oceanPosScale, in float3 i_oceanParams)
{
	return (i_samplePos - i_oceanPosScale.xy) / (i_oceanParams.x * i_oceanParams.y) + 0.5;
}

float3 WorldToUV(in float2 i_samplePos, in float3 i_oceanPosScale, in float3 i_oceanParams, in float i_sliceIndex)
{
...
)
{
	...
	const float radius0 = minScale / 2.0;
