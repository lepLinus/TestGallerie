...
{
	half ft = lerp(
		SAMPLE_TEXTURE2D(i_texture, sampler_Crest_linear_repeat, (1.25*i_worldXZUndisplaced + _CrestTime / 10.0) / (4.0*i_texelSize0*i_scale) + i_texelOffset * i_texelSize).x,
		SAMPLE_TEXTURE2D(i_texture, sampler_Crest_linear_repeat, (1.25*i_worldXZUndisplaced + _CrestTime / 10.0) / (4.0*i_texelSize1*i_scale) + i_texelOffset * i_texelSize).x,
		lodVal);
