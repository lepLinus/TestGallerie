	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 Frag(Varyings input) : SV_Target
			{
				...
				const float sceneZ01 = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uvDepth);
				...
				real3 sceneColour = SAMPLE_TEXTURE2D(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, input.grabPos.xy / input.grabPos.w).rgb;

#if _CAUSTICS_ON
				if (sceneZ01 != 0.0)
				{
					float2 scenePosNDC = uvDepth;
#if UNITY_UV_STARTS_AT_TOP
					scenePosNDC.y = 1. - scenePosNDC.y;
#endif
					float3 scenePos = ComputeWorldSpacePosition(scenePosNDC, sceneZ01, UNITY_MATRIX_I_VP);
