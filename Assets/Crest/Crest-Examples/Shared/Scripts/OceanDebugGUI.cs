...
/// <summary>
/// Handy debug overlay with some debug options and some useful shortcuts to speed up testing & development.
/// </summary>
public class OceanDebugGUI : MonoBehaviour
{
    ...
    public bool GetTargetVisible<SimType>() where SimType : LodDataMgr
    {
        var lodType = typeof(SimType);

        if (!_drawTargets.ContainsKey(lodType))
        {
            return false;
        }

        return _drawTargets[lodType];
    }

    public void SetTargetVisible<SimType>(bool visible) where SimType : LodDataMgr
    {
        var lodType = typeof(SimType);

        if (!_drawTargets.ContainsKey(lodType))
        {
            _drawTargets.Add(lodType, true);
        }

        _drawTargets[lodType] = visible;
    }
}
